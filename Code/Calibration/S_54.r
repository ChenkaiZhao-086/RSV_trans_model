...
S_54 <- parLapply(ParallelNodesInfo[[1]], PriorList, \(x) {
    setDTthreads(1)
    MCMC.MH(
        Prior = x, n_iterations = 100000, TargetDat = RefDat, lag = FALSE, Sus_reduce = c(0.5, 0.4),
        model = "SIR", AgeDistribution = TRUE
    )
})
save(S_54, file = "Output/S_54.RData")
Parallel.Stop()

# PosterioriCheck_S54 <- MCMC.PostProcess(S_54, burn_in = 0000, thin = 10)
