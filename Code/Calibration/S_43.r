...
S_43 <- parLapply(ParallelNodesInfo[[1]], PriorList, \(x) {
    setDTthreads(1)
    MCMC.MH(
        Prior = x, n_iterations = 100000, TargetDat = RefDat, lag = FALSE, Sus_reduce = c(0.4, 0.3),
        model = "SIR", AgeDistribution = TRUE
    )
})
save(S_43, file = "Output/S_43.RData")
Parallel.Stop()

# PosterioriCheck_S43 <- MCMC.PostProcess(S_43, burn_in = 0000, thin = 10)
