...
S_64 <- parLapply(ParallelNodesInfo[[1]], PriorList, \(x) {
    setDTthreads(1)
    MCMC.MH(
        Prior = x, n_iterations = 100000, TargetDat = RefDat, lag = FALSE, Sus_reduce = c(0.6, 0.4),
        model = "SIR", AgeDistribution = TRUE
    )
})
save(S_64, file = "Output/S_64.RData")
Parallel.Stop()

# PosterioriCheck_S64 <- MCMC.PostProcess(S_64, burn_in = 0000, thin = 10)
