...
S_63 <- parLapply(ParallelNodesInfo[[1]], PriorList, \(x) {
    setDTthreads(1)
    MCMC.MH(
        Prior = x, n_iterations = 100000, TargetDat = RefDat, lag = FALSE, Sus_reduce = c(0.6, 0.3),
        model = "SIR", AgeDistribution = TRUE
    )
})
save(S_63, file = "Output/S_63.RData")
Parallel.Stop()

# PosterioriCheck_S63 <- MCMC.PostProcess(S_63, burn_in = 0000, thin = 10)
