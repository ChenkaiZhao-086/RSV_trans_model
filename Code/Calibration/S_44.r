...
S_44 <- parLapply(ParallelNodesInfo[[1]], PriorList, \(x) {
    setDTthreads(1)
    MCMC.MH(
        Prior = x, n_iterations = 100000, TargetDat = RefDat, lag = FALSE, Sus_reduce = c(0.4, 0.4),
        model = "SIR", AgeDistribution = TRUE
    )
})
save(S_44, file = "Output/S_44.RData")
Parallel.Stop()

# PosterioriCheck_S44 <- MCMC.PostProcess(S_44, burn_in = 0000, thin = 10)
