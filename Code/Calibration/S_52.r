...
S_52 <- parLapply(ParallelNodesInfo[[1]], PriorList, \(x) {
    setDTthreads(1)
    MCMC.MH(
        Prior = x, n_iterations = 100000, TargetDat = RefDat, lag = FALSE, Sus_reduce = c(0.5, 0.2),
        model = "SIR", AgeDistribution = TRUE
    )
})
save(S_52, file = "Output/S_52.RData")
Parallel.Stop()

# PosterioriCheck_S52 <- MCMC.PostProcess(S_52, burn_in = 0000, thin = 10)
